...
# Configure Visual Studio Code using the adrianjuhl.visual_studio_code ansible role.
...
#  export ANSIBLE_ROLES_PATH=${THIS_SCRIPT_DIRECTORY}/../.ansible/roles/:${HOME}/.ansible/roles/
  ...
  ansible-playbook \
    "${playbook_command_options_array[@]}" \
    "${THIS_SCRIPT_DIRECTORY}/../.ansible/playbooks/configure_visual_studio_code.yml"
...
install_ansible_role_dependencies()
{
  ANSIBLE_ROLES_PATH=${HOME}/.ansible/roles/ \
      ansible-galaxy \
      install \
      "--role-file=${THIS_SCRIPT_DIRECTORY}/../.ansible/roles/requirements_visual_studio_code.yml" \
      --force
  last_command_return_code="${?}"
  if [ "${last_command_return_code}" -ne 0 ]; then
